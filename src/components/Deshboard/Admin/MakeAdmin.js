...
const MakeAdmin = () => {
    ...
    const handelMakeAdmin=(e)=>{
        ...
        const user={email}
        fetch('https://thawing-headland-16581.herokuapp.com/users/admin',
        {
            method:'PUT',
            headers:{
    ...
    return (
        <div className='flex justify-center flex-col items-center'>
          <form onSubmit={handelMakeAdmin} className="m-4 flex">
    	<input onBlur={handleOnBlur} 
        type='email'
         className="rounded-l-md w-96 p-4 border-t mr-0 border-b border-l text-gray-800 border-gray-200 bg-white"
          placeholder="admin@mail.com"/>
		<button
         type='submit' 
        className="px-8 rounded-r-lg bg-yellow-400  text-gray-800 font-bold p-4 uppercase border-yellow-500 border-t border-b border-r">Make a Admin</button>
